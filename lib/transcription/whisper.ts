...
let whisperPipelinePromise: Promise<AutomaticSpeechRecognitionPipeline> | null =
  null;
...
async function readPcm16WavAsFloat32(audioPath: string) {
  ...
  while (offset + 8 <= buffer.length) {
    ...
    if (chunkId === "fmt ") {
      ...
      if (audioFormat !== 1 || channels < 1 || bitsPerSample !== 16) {
        throw new Error("Audio must be PCM 16-bit WAV.");
      }
    }
  ...
  if (dataOffset < 0 || dataOffset + dataLength > buffer.length) {
    throw new Error("Invalid audio file.");
  }
  ...
  for (let index = 0; index < sampleCount; index++) {
    waveform[index] = buffer.readInt16LE(dataOffset + index * 2) / 32768;
  }
...
async function getWhisperPipeline() {
  if (!whisperPipelinePromise) {
    whisperPipelinePromise = (async () => {
      console.log(`[Whisper] Loading model: ${WHISPER_MODEL}`);
...
export async function transcribeAudioFile(audioPath: string): Promise<string> {
  ...
  try {
    const waveform = await readPcm16WavAsFloat32(audioPath);

    console.log(`[Whisper] Audio samples: ${waveform.length}`);
